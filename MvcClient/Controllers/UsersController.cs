using System.Text.Json;
using Microsoft.AspNetCore.Mvc;
using Uam.AdvancedProgramming.MvcClient.Models;

namespace Uam.AdvancedProgramming.MvcClient.Controllers;

public class UsersController : BaseApiController
{
    private readonly IConfiguration _configuration;

    private static readonly JsonSerializerOptions JsonOptions = new()
    {
        PropertyNameCaseInsensitive = true
    };

    public UsersController(
        IHttpClientFactory httpClientFactory,
        IConfiguration configuration
    ) : base(httpClientFactory, configuration)
    {
        _configuration = configuration;
    }

    public IActionResult Index()
    {
        if (string.IsNullOrWhiteSpace(Request.Cookies["AccessToken"]))
        {
            return RedirectToAction("Login", "Auth");
        }

        return View();
    }

    [HttpGet]
    public async Task<IActionResult> GetUsers(CancellationToken cancellationToken)
    {
        var endpoint = $"{_configuration["ApiSettings:BaseUrl"]}{_configuration["ApiSettings:UsersBaseEndpoint"]}/GetAllUsers";

        var response = await SendApiRequestAsync(HttpMethod.Get, endpoint, null, cancellationToken);

        if (response is null)
        {
            return Unauthorized();
        }

        var content = await response.Content.ReadAsStringAsync(cancellationToken);

        if (!response.IsSuccessStatusCode)
        {
            return StatusCode((int)response.StatusCode, content);
        }

        var apiResult = JsonSerializer.Deserialize<ApiResponse<List<UserDto>>>(content, JsonOptions);

        return Json(apiResult?.Result ?? new List<UserDto>());
    }

    [HttpGet]
    public async Task<IActionResult> GetRoles(CancellationToken cancellationToken)
    {
        var endpoint = $"{_configuration["ApiSettings:BaseUrl"]}{_configuration["ApiSettings:RolesBaseEndpoint"]}/GetAllRoles";

        var response = await SendApiRequestAsync(HttpMethod.Get, endpoint, null, cancellationToken);

        if (response is null)
        {
            return Unauthorized();
        }

        var content = await response.Content.ReadAsStringAsync(cancellationToken);

        if (!response.IsSuccessStatusCode)
        {
            return StatusCode((int)response.StatusCode, content);
        }

        var apiResult = JsonSerializer.Deserialize<ApiResponse<List<RoleDto>>>(content, JsonOptions);

        var activeRoles = apiResult?.Result?.Where(x => x.IsActive).ToList()
                         ?? new List<RoleDto>();

        return Json(activeRoles);
    }

    [HttpPost]
    public async Task<IActionResult> CreateUser([FromBody] UserUpsertDto dto, CancellationToken cancellationToken)
    {
        var endpoint = $"{_configuration["ApiSettings:BaseUrl"]}{_configuration["ApiSettings:UsersBaseEndpoint"]}/CreateUser";

        var response = await SendApiRequestAsync(HttpMethod.Post, endpoint, dto, cancellationToken);

        if (response is null)
        {
            return Unauthorized();
        }

        var content = await response.Content.ReadAsStringAsync(cancellationToken);

        return StatusCode((int)response.StatusCode, content);
    }

    [HttpPut]
    public async Task<IActionResult> UpdateUser(int id, [FromBody] UserUpsertDto dto, CancellationToken cancellationToken)
    {
        var endpoint = $"{_configuration["ApiSettings:BaseUrl"]}{_configuration["ApiSettings:UsersBaseEndpoint"]}/UpdateUser/{id}";

        var response = await SendApiRequestAsync(HttpMethod.Put, endpoint, dto, cancellationToken);

        if (response is null)
        {
            return Unauthorized();
        }

        var content = await response.Content.ReadAsStringAsync(cancellationToken);

        return StatusCode((int)response.StatusCode, content);
    }

    [HttpDelete]
    public async Task<IActionResult> DeleteUser(int id, CancellationToken cancellationToken)
    {
        var endpoint = $"{_configuration["ApiSettings:BaseUrl"]}{_configuration["ApiSettings:UsersBaseEndpoint"]}/DeleteUser/{id}";

        var response = await SendApiRequestAsync(HttpMethod.Delete, endpoint, null, cancellationToken);

        if (response is null)
        {
            return Unauthorized();
        }

        var content = await response.Content.ReadAsStringAsync(cancellationToken);

        return StatusCode((int)response.StatusCode, content);
    }
}